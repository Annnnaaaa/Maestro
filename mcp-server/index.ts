...
async function fetchJson<T extends JsonValue>(
  url: string,
  init?: RequestInit
...
function asToolResult(obj: unknown) {
...
function toolError(message: string, details?: unknown) {
  return {
    ...
    structuredContent: { error: message, details } as Record<string, unknown>,
  };
}
...
async function getManifest(base: string): Promise<JsonValue> {
  // Requested path is /api/maestro/manifest; Maestro currently exposes /api/agent/manifest.
  const primary = await fetchJson<JsonValue>(`${base}/api/maestro/manifest`);
  if (primary.ok) return primary.data;
  const fallback = await fetchJson<JsonValue>(`${base}/api/agent/manifest`);
  if (fallback.ok) return fallback.data;
  throw new Error(
    `manifest fetch failed (maestro/manifest=${primary.status}, agent/manifest=${fallback.status})`
  );
}
...
async function jobStatus(base: string, jobId: string): Promise<JsonValue> {
  ...
  let last: { ok: false; status: number; text: string } | null = null;
  for (const url of urls) {
    const res = await fetchJson<JsonValue>(url, { method: "GET" });
...
function isDoneStatus(payload: JsonValue): { done: boolean; finalUrl?: string } {
  ...
  const direct =
    ...
    (typeof (p.result as any)?.video_url === "string" && (p.result as any).video_url);

  if (typeof p.done === "boolean" && p.done === true) return { done: true, finalUrl: direct };
...
server.registerTool(
  "maestro_get_manifest",
  {
    ...
    description:
      "Get the capability manifest for Maestro, an orchestrator agent that generates product videos by hiring specialist sub-agents and paying them via Lightning Network.",
  ...
  async () => {
    try {
      ...
      return toolError("Failed to get manifest", { message: (e as Error).message });
    }
  }
);
...
server.registerTool(
  "maestro_submit_job",
  {
    ...
    description:
      "Submit a job to Maestro. Provide product info and visual context, optionally pass a github_repo_url to have Maestro gather missing context autonomously.",
    inputSchema: z
      .object({
  ...
  async (args) => {
    try {
      ...
      const res = await fetchJson<JsonValue>(`${base}/api/maestro/job`, {
        method: "POST",
      ...
      return toolError("Failed to submit job", { message: (e as Error).message });
    }
  }
);
...
server.registerTool(
  ...
  async ({ invoice }) => {
    const base = getBaseUrl();
    try {
      const res = await fetchJson<JsonValue>(`${base}/api/lightning/pay`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ invoice }),
      });

      if (res.ok) return asToolResult(res.data);
      ...
      return toolError("Invoice payment failed", { message: (e as Error).message });
    }
  }
);
...
server.registerTool(
  ...
  async ({ job_id }) => {
    try {
      ...
      return toolError("Failed to fetch job status", { message: (e as Error).message });
    }
  }
);
...
server.registerTool(
  ...
  async ({ job_id }) => {
    try {
      ...
      return toolError("Failed to execute job", { message: (e as Error).message });
    }
  }
);
